#!/usr/bin/env bash
# Idempotente Anlage von Paperless Saved Views via API.
# Liest Credentials aus .secrets/paperless.env (Mode 600).
# Trennung: dieses Script kuemmert sich nur um Saved Views,
# Baseline-Daten (Tags/Doctypes/Korrespondenten) -> setup-paperless-baseline.sh.
set -euo pipefail
cd "$(dirname "$0")/.."
...
"$VENV/bin/python" scripts/setup_paperless_saved_views.py
